#!/usr/bin/env python3
"""
Terminal 2 — VEHICLE-TRUCK2 더미 (CACC 팔로워 | 텔레메트리 전용)

표시 내용:
  - vehicle-truck2 컨테이너 배너 (OpenClaw 없음)
  - .transfer/telemetry.log 를 실시간 스트리밍
    (CARLA 시나리오가 100틱마다 기록하는 속도/간격/상태값)
...
def draw_static():
    st = container_status("vehicle-truck2")
    dot = c("green", "●") if "Up" in st else c("yellow", "○")
    print(f"  {dot}  vehicle-truck2  {c('dim', st)}")
