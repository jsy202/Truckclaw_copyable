#!/usr/bin/env python3
"""
Terminal 1 — VEHICLE-TRUCK1 DinD 모니터 (TX side)

상태 전환:
  IDLE     : 시나리오 대기 중
  TELEMETRY: .transfer/telemetry.log 실시간 표시 (시나리오 시작 후)
  TX_ACTIVE: 복제 트리거 → .transfer/tx/.progress.log 스트리밍으로 전환
...
def draw_header():
    ...
    print(c("bold", f"┌{ln}┐"))
    print(c("bold", "│") +
          f"  VEHICLE-TRUCK1  [{c('cyan','리더')} {c('bold','│')} {c('cyan','DinD 모니터')}]")
    print(c("bold", f"└{ln}┘"))
    print()
...
def main():
    ...
    while True:
        # ── TX 로그에서 센티널(=== TX START ===) 감지 → TX 모드 전환 ────────
        if state != "tx" and TX_LOG.exists():
            try:
                if (TX_LOG.stat().st_mtime > SCRIPT_START and
                    ...
                    draw_inner_status()
                    print(c("bold", "═" * 62))
                    print(c("bold", "  OpenClaw 복제 TX 시작  (truck1 ──▶ truck3)"))
                    print(c("bold", "═" * 62))
                    print()
                    state  = "tx"
                    tx_pos = 0
        ...
        if state == "tx":
            ...
            if chunk:
                # 센티널 라인만 제거하고 나머지는 raw 출력 (\r 애니메이션 보존)
