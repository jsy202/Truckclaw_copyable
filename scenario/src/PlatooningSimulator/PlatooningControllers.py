...
class FollowerController(LowLevelController):
    ...
    def compute_control(self, lead_waypoints, index):  # index is ego vehicle index
        ...
        if len(lead_waypoints) == 0:
            # Keep followers alive if the shared deque briefly runs dry.
            lead_waypoints.append(self.platoon.map.get_waypoint(self.platoon[0].get_location()))

        _passed = min(_passed, len(lead_waypoints) - 1)
